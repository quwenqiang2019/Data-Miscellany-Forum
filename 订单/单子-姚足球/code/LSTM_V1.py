...
df.insert(0, '当根K线收益率', df.pop('当根K线收益率'))
df = df.dropna()
df['当根K线收益率'] = df['当根K线收益率'].str.rstrip('%').astype(float)/100
print(df.shape)
print(df.head())
...
plt.plot(df_for_training['当根K线收益率'], label='Training Data')
plt.plot(df_for_testing['当根K线收益率'], label='Testing Data')
plt.xlabel('时间')
plt.xticks(rotation=45)
plt.ylabel('当根K线收益率')
plt.title('训练集和测试集')
plt.legend()
plt.savefig(os.path.join(base_dir, 'result', 'Train_and_Test_v1.jpg'), bbox_inches='tight', dpi = 600)
plt.show()
...
plt.ylabel('当根K线收益率')
plt.legend()
plt.savefig(os.path.join(base_dir, 'result', 'lstm_v1_pred.jpg'), bbox_inches='tight', dpi = 600)
plt.show()
...
df.to_excel(os.path.join(base_dir, 'result', 'lstm_v1.xlsx'), index=False)
