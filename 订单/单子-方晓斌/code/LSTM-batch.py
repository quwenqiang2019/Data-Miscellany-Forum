...
def lstm_model(name):
    ...
    # 读取数据集
    filename = f'{name}_2020-2024.xlsx'
    ...
    plt.legend()
    plt.savefig(os.path.join(base_dir, 'result', f'{name}_lstm_pred_train.jpg'), bbox_inches='tight', dpi = 600)
    plt.show()
    ...
    plt.legend()
    plt.savefig(os.path.join(base_dir, 'result', f'{name}_lstm_pred_test.jpg'), bbox_inches='tight', dpi = 600)
    plt.show()
    ...
    # 读取数据集
    filename = f'{name}_20230517-20241231.xlsx'
    if os.path.exists(os.path.join(base_dir, 'data', "qpe_zd", filename)):
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__name__)))
        ...
        merged_df.to_excel(os.path.join(base_dir, 'result', f'{name}_result_compare.xlsx'))
        ...
        plt.legend()
        plt.savefig(os.path.join(base_dir, 'result', f'{name}_lstm_pred_compare.jpg'), bbox_inches='tight', dpi = 600)
        plt.show()
    else:
        pass
