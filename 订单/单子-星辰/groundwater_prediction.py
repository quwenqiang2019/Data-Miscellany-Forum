...
def plot_metrics_comparison(results, filename):
    ...
    for idx, metric in enumerate(metrics):
        ...
        for i, model in enumerate(models):
            values = [results[well][model][metric] for well in wells]
...
def train_and_evaluate(well_name, well_df, aux_df):
    ...
    # feature_cols = ['降雨量(mm/天)', '日平均气温(°C)', '逐日蒸散发（mm）', '地下水开采量', '农田灌溉用水量']
    ...
    for model_name, model in models.items():
        ...
        print(f"  R2: {metrics['R2']:.4f}, RMSE: {metrics['RMSE']:.4f}, MAE: {metrics['MAE']:.4f}, MAPE: {metrics['MAPE']:.2f}%")
    ...
    return results, predictions, y_true.flatten()
