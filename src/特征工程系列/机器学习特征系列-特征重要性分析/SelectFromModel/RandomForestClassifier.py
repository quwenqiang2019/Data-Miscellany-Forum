...
# 方法1：使用默认阈值（均值）
selector_mean = SelectFromModel(rf_model, threshold='mean', prefit=True)
...
# 方法2：使用中位数阈值
selector_median = SelectFromModel(rf_model, threshold='median', prefit=True)
...
selector_custom = SelectFromModel(rf_model, threshold=0.05, prefit=True)
...
colors = ['#e74c3c' if imp > feature_importance['importance'].mean() else '#3498db' 
...
ax1.axvline(feature_importance['importance'].mean(), color='red', linestyle='--', linewidth=2,
            label=f'Mean Threshold ({feature_importance["importance"].mean():.3f})')
