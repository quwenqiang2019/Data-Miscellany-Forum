...
class BiLSTM(nn.Module):
    def __init__(
        self, input_dim=5, lstm_hidden=128, lstm_layers=2, pred_len=5, dropout=0.2
    ):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(lstm_hidden, pred_len),
        )
...
def visualize_results_v2(idx, preds, trues):
    ...
    sns.histplot(
        resid, bins=40, stat="density", color=vivid_colors[3], kde=True, alpha=0.7
    )
    plt.axvline(
        np.mean(resid),
        color=vivid_colors[0],
    ...
    # rolling = np.array(
    #     [mape_series[i : i + win].mean() for i in range(0, len(mape_series) - win + 1)]
    # )
...
if __name__ == "__main__":
    ...
    # 数据加载
    df = pd.read_csv(
        "~/snap/wenqiang/Data-Miscellany-Forum/src/多变量时序预测系列-股票预测/多输入+单输出+多步/data.csv",
        parse_dates=["Date"],
        index_col=[0],
    )
    df = pd.DataFrame(df)
    ...
    print(
        f"Training samples: {len(df_for_training)}, Testing samples: {len(df_for_testing)}"
    )
    ...
    train_dataset = TimeSeriesDataset(
        df_for_training_scaled, seq_len=seq_len, pred_len=pred_len
    )
    test_dataset = TimeSeriesDataset(
        df_for_testing_scaled, seq_len=seq_len, pred_len=pred_len
    )
    ...
    preds_test = scaler.inverse_transform(
        np.reshape(preds_copies_array, (len(y_pred), int(var_num)))
    )[:, 0]
    ...
    trues_test = scaler.inverse_transform(
        np.reshape(trues_copies_array, (len(y_true), int(var_num)))
    )[:, 0]
