...
trainX = np.reshape(trainX, (trainX.shape[0], window_size,1,1, fea_num))
testX = np.reshape(testX, (testX.shape[0],window_size, 1,1, fea_num))
...
# 初始化顺序模型
my_model = Sequential()
my_model.add(ConvLSTM2D(filters=64, kernel_size=(1,1), activation='relu', input_shape=(window_size, 1,1, fea_num)))
my_model.add(Flatten())
my_model.add(Dense(1))
my_model.compile(loss='mse', optimizer='adam')
my_model.fit(trainX, trainY, epochs=50)
# 打印模型
my_model.summary()
