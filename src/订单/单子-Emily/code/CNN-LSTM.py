...
def df_converse(train_data, test_data):
    ...
    trainX = np.reshape(trainX, (trainX.shape[0], 1, window_size, 1))
    testX = np.reshape(testX, (testX.shape[0], 1, window_size, 1))
...
def model_predict(input_value):
    ...
    inputX = np.reshape(inputX, (inputX.shape[0], 1, window_size, 1))
