...
# 读取数据
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
folder_path = os.path.join(base_dir, 'data', 'qpe')
# 获取文件夹中所有文件
files = os.listdir(folder_path)
print(files)
...
for station, data in stations_data.items():
    # 构建保存路径
    output_file = f'{station}_20230517-20241231.xlsx'
    # 保存为 Excel 文件
    data.to_excel(os.path.join(base_dir, 'data', 'qpe_zd',output_file), index=False)
    print(f'Saved data for station {station} to {output_file}')