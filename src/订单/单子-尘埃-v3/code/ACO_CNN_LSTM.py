...
def build_cnn_lstm_model(X_train, filters, kernel_size, neurons):

    nb_features = X_train.shape[2]
    input1 = X_train.shape[1]
    inputs = Input(shape=(input1, nb_features))
    model = Conv1D(filters=filters, kernel_size=kernel_size, activation='relu')(inputs)  # 卷积层
    model = MaxPooling1D(pool_size=1)(model)
    model = LSTM(neurons, activation='tanh', return_sequences=True)(model)  # LSTM层
    model = Flatten()(model)
...
filters = int(aco.g_best[0])
kernel_size = int(aco.g_best[1])
neurons = int(aco.g_best[2])
batch_size = int(aco.g_best[3])

model = build_cnn_lstm_model(trainX, filters, kernel_size, neurons)


history1 = model.fit(trainX, trainY, epochs=50, batch_size=batch_size, validation_split=0.2, verbose=1,
...
plt.ylabel('B')
plt.legend()
plt.savefig(os.path.join(base_dir, 'result', 'aco_cnn_lstm_pred_train.jpg'), bbox_inches='tight', dpi = 600)
plt.show()
...
plt.ylabel('B')
plt.legend()
plt.savefig(os.path.join(base_dir, 'result', 'aco_cnn_lstm_pred_test.jpg'), bbox_inches='tight', dpi = 600)
plt.show()
...
df.to_excel(os.path.join(base_dir, 'result', 'aco_cnn_lstm.xlsx'), index=False)