...
def data_read(filename):
    df = pd.read_csv(os.path.join(base_dir, 'data', filename), encoding='gb2312')
    df = pd.DataFrame(df)
    df = df.iloc[:, 0:26]  # 选择需要的列，这里选择了前26列，如果增加了其他变量这里需要做改动
    df = df.dropna()
...
def data_spilt(df):
    ...
    plt.title(target)
    plt.legend()
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'train_and_test_split.jpg'), bbox_inches='tight', dpi=600)
    plt.show()

    return df_for_training, df_for_testing


def data_process(df_for_training, df_for_testing):
...
def plot_heatmap(df):
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'heatmap.jpg'), bbox_inches='tight', dpi=1600)
    plt.show()
...
def build_LSTM_model(neurons_1, neurons_2, dropout_rate):   # 这里采用sequential模型，也可以采用其他模型进行网络的搭建,模型的结构可以自行做调整
    grid_model = Sequential()
    grid_model.add(Input(shape=(window_size, fea_num)))
    grid_model.add(LSTM(neurons_1, return_sequences=True))
...
def build_Bi_LSTM_model(neurons_1, neurons_2, dropout_rate):   # 这里采用sequential模型，也可以采用其他模型进行网络的搭建,模型的结构可以自行做调整
    grid_model = Sequential()
    grid_model.add(Input(shape=(window_size, fea_num)))
...
def model_prediction(my_model):
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'pred_train.jpg'), bbox_inches='tight', dpi = 600)
    plt.show()
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'pred_test.jpg'), bbox_inches='tight', dpi = 600)
    plt.show()

    return original_train, pred_train, original_test, pred_test


def result_analysis(original_test, pred_test):
    df0 = pd.DataFrame({'日期': df_for_testing.index[window_size:],'真实值': original_test, '预测值': pred_test})
    print(df0)
    # 计算当天预测股价和前一天真实股价的比较结果,也就是第二天的预测值和当天的真实值进行对比。如果上涨标注1，下跌标注-1
    ...
    # 定义映射关系的字典
    mapping = {0: -1, 1: 1}
    df0['比较结果'] = df0['比较结果'].map(mapping)
    print(df0)
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    df0.to_excel(os.path.join(base_dir, 'result', model_name, '预测值对比前一天真实值.xlsx'), index=False)   # 将比较结果存为数据表


def model_evalution(original_train, pred_train, original_test, pred_test):
    # 计算误差
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    df.to_excel(os.path.join(base_dir, 'result', model_name, 'evalution.xlsx'), index=False)   # 将评估指标值存为数据表

    return testScore1,  testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4



def plot_radar(testScore1,  testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4):
    values1 = [round(trainScore1, 2) / max(round(trainScore1, 2), round(testScore1, 2)),
                  round(trainScore2, 2) / max(round(trainScore2, 2), round(testScore2, 2)),
                  round(trainScore3, 2) / max(round(trainScore3, 2), round(testScore3, 2)),
                  round(trainScore4, 2) / max(round(trainScore4, 2), round(testScore4, 2))]
    values2 = [round(testScore1, 2) / max(round(trainScore1, 2), round(testScore1, 2)),
                  round(testScore2, 2) / max(round(trainScore2, 2), round(testScore2, 2)),
                  round(testScore3, 2) / max(round(trainScore3, 2), round(testScore3, 2)),
                  round(testScore4, 2) / max(round(trainScore4, 2), round(testScore4, 2))]
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'Comparison_radar.jpg'), bbox_inches='tight', dpi=600)
    plt.show()


def plot_bar(testScore1,  testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4):
    font = {'family': 'Times New Roman',
            'size': 12,
            }
    sns.set(font_scale=1.2)
    ...
    if not os.path.exists(os.path.join(base_dir, 'result', model_name)):
        os.makedirs(os.path.join(base_dir, 'result', model_name))
    plt.savefig(os.path.join(base_dir, 'result', model_name, 'Comparison_bar.jpg'), bbox_inches='tight', dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    df = data_read("沪深300.csv")
    fea_num = len(df.columns)  # 计算一下数据的变量数量，也就是列数
    plot_heatmap(df)
    df_for_training, df_for_testing = data_spilt(df)
    scaler = MinMaxScaler(feature_range=(0, 1))  # 定义归一化对象
    trainX, testX, trainY, testY = data_process(df_for_training, df_for_testing)

    if model_name == 'LSTM':
        my_model = model_fit(build_LSTM_model)
    elif model_name == 'Bi-LSTM':
        my_model = model_fit(build_Bi_LSTM_model)

    original_train, pred_train, original_test, pred_test = model_prediction(my_model)
    result_analysis(original_test, pred_test)
    testScore1, testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4 = model_evalution(
        original_train, pred_train, original_test, pred_test)
    plot_bar(testScore1, testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4)
    plot_radar(testScore1, testScore2, testScore3, testScore4, trainScore1, trainScore2, trainScore3, trainScore4)
