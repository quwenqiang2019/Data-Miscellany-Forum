...
def NB(X_train, y_train, X_test, y_test):
    ...
    predict_target=GNB1.predict(X_test)
    predict_target_prob=GNB1.predict_proba(X_test)  # 输出分类概率
    predict_target_prob_gnb = predict_target_prob[:, 1]

    print(sum(predict_target==y_test))
    print(metrics.classification_report(y_test,predict_target))
    print(metrics.confusion_matrix(y_test, predict_target))

    return list(y_test), list(predict_target_prob_gnb)
...
if __name__=="__main__":
    ...
    y_test, predict_target_prob_gnb=NB(X_train, y_train, X_test, y_test)
    Draw_ROC(y_test,predict_target_prob_gnb)
