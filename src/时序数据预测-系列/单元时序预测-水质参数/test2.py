...
# 读取数据集
data = pd.read_excel('模拟测试.xlsx')
data = pd.DataFrame(data)
# 将日期列转换为日期时间类型
data['日期'] = pd.to_datetime(data['日期'], format='%Y%m%d', infer_datetime_format=True)
# 使用插值法填充缺失值
data['TSM值'] = data['TSM值'].interpolate()
print(data['TSM值'])
start_time = data['日期'].min()
end_time = data['日期'].max()
date = pd.date_range(start=start_time, end=end_time, freq='15D')
print(date[:-1])

data = pd.DataFrame(data['TSM值'].values, index=date[:-1], columns=['TSM值'])
print(data)

# # # 将日期列设置为索引
# # data.set_index('日期', inplace=True)
# # # # 构造规律的时间间隔
# # data = data.resample('15D').asfreq()
# # # # 使用插值法填充缺失值
# # # data['TSM值'] = data['TSM值'].interpolate()
# # print(data)
# # print(data.index)
...
# predictions = model_fit.predict(start='2019-03-15', end='2019-09-01')
