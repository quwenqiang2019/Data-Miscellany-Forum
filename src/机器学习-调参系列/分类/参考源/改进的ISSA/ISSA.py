...
class ISSA():
    def __init__(self, func, n_dim=None, pop_size=20, max_iter=50, lb=-512, ub=512, verbose=False):
        ...
        # ★★改进1：Cat映射种群初始化★★
        xx = np.random.rand(pop_size, n_dim)
        yy = np.random.rand(pop_size, n_dim)

        for i in range(pop_size):
            for j in range(n_dim-1):
                xx[i, j+1] = (xx[i, j] + yy[i, j]) % 1
                yy[i, j+1] = (xx[i, j] + 2*yy[i, j]) % 1
        self.X = np.zeros((pop_size, n_dim))
        for i in range(pop_size):
            self.X[i] = (np.array(ub) - np.array(lb)) * yy[i] + np.array(lb)
    ...
    def detect(self, iter_num):
        ...
        # ★★改进2：步长因子动态调整★★
        beta = min(self.Y) - (min(self.Y) - max(self.Y)) * ((self.max_iter-iter_num)/self.max_iter) ** 1.5
        K = (min(self.Y) - max(self.Y)) * np.exp(-20*np.tan((iter_num/self.max_iter)**2))*(2*np.random.rand(1)-1)
        ...
        for j in range(len(b)):
            # ★★改进3：Levy飞行★★
            beta = 1.5
            sigma_u = (gamma(1 + beta).rvs() * np.sin(np.pi * beta / 2) / (
                    gamma((1 + beta) / 2).rvs() * beta * 2 ** ((beta - 1) / 2))) ** (1 / beta)
            sigma_v = 1
            u = norm(loc=0, scale=sigma_u).rvs()
            v = norm(loc=0, scale=sigma_v).rvs()
            levy = u / (abs(v)) ** (1 / beta)
            if self.Y[b[j]] > self.gbest_y:

                # self.X[b[j], :] = self.gbest_x + norm.rvs(size=self.n_dim) * np.abs(self.X[b[j], :] - self.gbest_x)
                self.X[b[j], :] = levy * self.gbest_x + beta * np.abs(self.X[b[j], :] - levy * self.gbest_x)
            else:
                # self.X[b[j], :] = self.X[b[j], :]+(2*np.random.rand(1)-1)*np.abs(self.X[b[j], :]-self.x_max)/(self.func(self.X[b[j]])-self.y_max+e)
                self.X[b[j], :] = self.X[b[j], :] + K * np.abs(self.X[b[j], :] - self.x_max) / (self.func(self.X[b[j]]) - self.y_max + e)
...
#     pop_size = 100
#     max_iter = 100
#     ssa = ISSA(demo_func, n_dim=n_dim, pop_size=pop_size, max_iter=max_iter, lb=lb, ub=ub)
#     ssa.run()
