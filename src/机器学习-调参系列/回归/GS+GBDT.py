...
if __name__ == '__main__':
    ...
    # plt.rcParams["axes.unicode_minus"] = False
    #
    # plt.scatter(list(np.array(y_test_base_gb).flatten()), list(np.array(y_pred_base_gb).flatten()), s=50, marker='*', c='purple', label='GB基准模型')
    # plt.scatter(list(np.array(y_test_gs_gb).flatten()), list(np.array(y_pred_gs_gb).flatten()), s=50, marker='+', c='orange',label='网格搜索优化GB模型')
    #
    # x2 = np.arange(np.min(y_test_base_gb), np.max(y_test_base_gb), 0.1)
    # y2 = x2
