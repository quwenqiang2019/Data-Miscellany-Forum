...
hidden1 = Dense(units=hidden_size, activation='relu')(input)
hidden2 = Dense(units=hidden_size, activation='relu')(hidden1)
output = Dense(units=output_size, activation='softmax')(hidden2)
...
#
#     # 测试部分（每个epoch下均进行一次计算），total_correct为预测对的样本个数, total_number为测试的总样本数，将这两个变量都初始化为0
